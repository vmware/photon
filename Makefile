...
PHOTON_KEEP_RPMCHECK_OPTION := -q
else ifeq ($(RPMCHECK),stop_on_error)
PHOTON_RPMCHECK_OPTION := -u
else
PHOTON_RPMCHECK_OPTION :=
PHOTON_KEEP_RPMCHECK_OPTION :=
endif
...
check: packages
ifeq ($(RPMCHECK),stop_on_error) 
	$(eval PHOTON_KEEP_RPMCHECK_OPTION = )
else
	$(eval PHOTON_KEEP_RPMCHECK_OPTION = -q) 
endif
