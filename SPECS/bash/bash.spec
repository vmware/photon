...
if [ -d %{_usr}/local/lib/pkgconfig ]; then
  pathappend %{_usr}/local/lib/pkgconfig PKG_CONFIG_PATH
fi
if [ -d %{_usr}/local/bin ]; then
  pathprepend %{_usr}/local/bin
fi
if [ -d %{_usr}/local/sbin -a $EUID -eq 0 ]; then
  pathprepend %{_usr}/local/sbin
fi
EOF
...
%changelog
* Mon Apr 08 2024 Prashant S Chauhan <prashant.singh-chauhan@broadcom.com> 5.0-5
