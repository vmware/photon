...
if [[ -z "$EMQ_IDLE_TIME" ]]; then
    export EMQ_IDLE_TIME=5
fi
...
IDLE_TIME=0
while [[ $IDLE_TIME -lt $EMQ_IDLE_TIME ]]
do
    IDLE_TIME=$((IDLE_TIME+1))
    if [[ ! -z "$(/opt/emqttd/bin/emqttd_ctl status |grep 'is running'|awk '{print $1}')" ]]; then
        IDLE_TIME=0
    else
        echo "['$(date -u +"%Y-%m-%dT%H:%M:%SZ")']:emqttd not running, waiting for recovery in $((EMQ_IDLE_TIME*5-IDLE_TIME*5)) seconds"
    fi
    sleep 5
done
