...
cmake -DCMAKE_INSTALL_PREFIX:PATH=%{_prefix} -DCLANG-FORMAT:FILEPATH=clang-format -DENVOY_COTIRE_MODULE_DIR:FILEPATH=%{_builddir}/%{name}-%{version} -DENVOY_PROTOBUF_INCLUDE_DIR:FILEPATH=%{_includedir} -DENVOY_PROTOBUF_PROTOC:FILEPATH=%{_bindir}/protoc -DENVOY_EXE_EXTRA_LINKER_FLAGS:STRING=-L%{_libdir} -DENVOY_TEST_EXTRA_LINKER_FLAGS:STRING=-L%{_libdir} -DENVOY_DEBUG:BOOL=OFF -DENVOY_STRIP:BOOL=ON ..
...
%changelog
*    Wed Jun 14 2017 Harish Udaiya Kumar <hudaiyakumar@vmware.com> 1.2.0-1
