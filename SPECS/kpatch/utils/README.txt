...
auto_livepatch [options] -p [list of patch files]
...
Simplest Usage - Build livepatch for current kernel
    ...
    auto_livepatch -p example.patch


Set non-replace flag and save debug information

    auto_livepatch -p example.patch -R --exportdebuginfo


Multiple patches

    auto_livepatch -p example1.patch example2.patch ... exampleN.patch


Build module for Photon 3.0 aws flavor, with a set name and a set output directory

    auto_livepatch -k 4.19.245-5.ph3-aws -p example.patch -n klp_module.ko -o livepatch_dir


Build module with description field

    auto_livepatch -p example_patch -d description.txt

Package module as RPM

    auto_livepatch -p example_patch -d description.txt -k 5.10.118-3.ph3 --rpm --rpm-version 2 --rpm-release 3 --rpm-desc rpm_description.txt

Photon 4.0 rt flavor - All options set

    auto_livepatch -p example_patches/example.patch -k 5.10.118-rt67-3.ph4-rt -o test_dir -n test -d description.txt -R --export-debuginfo
...
Simplest Usage - Build livepatch for current kernel.
    ...
    gen_livepatch -p example.patch


Set non-replace flag and save debug information.

    gen_livepatch -p example.patch -R --exportdebuginfo


Multiple patches.

    gen_livepatch -p example1.patch example2.patch ... exampleN.patch


Build module with description field

    gen_livepatch -p example_patch -d description.txt

Package module as RPM

    gen_livepatch -p example_patch -d description.txt -k 5.10.118-3.ph3 --rpm --rpm-version 2 --rpm-release 3 --rpm-desc rpm_description.txt

Native module - All options set.

    gen_livepatch -p example_patches/example.patch -o test_dir -n test -d description.txt -R --export-debuginfo

Specify kernel version (will fail if Photon versions don't match)

    gen_livepatch -k 4.19.245-5.ph3-aws -p example.patch
