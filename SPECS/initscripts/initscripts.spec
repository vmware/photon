...
%install
make ROOT=%{buildroot} SUPERUSER=`id -un` SUPERGROUP=`id -gn` mandir=%{_mandir} install %{?_smp_mflags}
...
rm -f %{buildroot}%{_sysconfdir}/rc.d/rc.local \
      %{buildroot}%{_sysconfdir}/rc.local \
      %{buildroot}%{_sbindir}/ifdown \
      %{buildroot}%{_sbindir}/ifup
...
- network_function: return immediately when device is present
- add configurable DEVTIMEOUT
- fedora-import-state: do not clobber /
- network-functions: grep->fgrep in bonding masters matching
...
- Clean up entire buildroot directory in %%clean.
