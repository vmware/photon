...
export GOPATH="$(pwd)"
cd ..
mv "${GOPATH}" cni-plugin
mkdir -p "${GOPATH}/src/github.com/projectcalico"
mv cni-plugin "${GOPATH}/src/github.com/projectcalico/"
...
install -vdm 0755 %{buildroot}%{_datadir}/calico-cni/k8s
install -vpm 0755 -t %{buildroot}%{_datadir}/calico-cni/k8s/ k8s-install/scripts/install-cni.sh
install -vpm 0755 -t %{buildroot}%{_datadir}/calico-cni/k8s/ k8s-install/scripts/calico.conf.default
...
%changelog
*   Mon Oct 16 2017 Bo Gan <ganb@vmware.com> 1.10.0-2
-   cleanup GOPATH
