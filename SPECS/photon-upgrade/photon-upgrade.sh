...
function show_help() {
  ...
  builtin echo -n "
Usage: $PROG [--repos=r1,...] [--upgrade-os] [--to-ver=VERSION] [--assume-yes] [--skip-update] [--install-all] [--rm-pkgs-pre=p1,p2,...] [--rm-pkgs-post=p1,p2,...] [--precheck-only]
...
--to-ver       : The Photon OS version to upgrade to, default value of this
                 option is 6.0 when not provided.
...
function check_packages_in_target_repo() {
  ...
  # Even if above command fails, the online repo lookup will be falled back to
  # in the below for loop as "${list_available_fn}" me be empty
  ${SED} -i -E 's#^(.*)\.[^.]+$#\1#' "${list_available_fn}"

  for ptgt in $*; do
    [ ${#ptgt} -gt 10 -a "${ptgt: -10}" = '-debuginfo' ] && continue
    $GREP -qw "^${ptgt}$" "${list_available_fn}" &> /dev/null && continue
    ${TDNF} $REPOS_OPT "--releasever=$TO_VERSION" list available \
...
# Installed packages which were replaced by other packages or installed packages
...
# depend on one or more of the packages from the argument list
# Returns:
#   0 - success, when the erased packages are only of Photon OS (*.ph5.*rpm)
...
case "$TO_VERSION" in
  ...
  6.0 )
    ...
    ;;
  * )
    echoerr "Valid values for --to-ver can only be 6.0"
    show_help $ERETRY_EINVAL
    ;;
esac
