...
function show_help() {
  ...
  echo -n "
Usage: $PROG [--repos=r1,...] [--upgrade-os] [--to-ver=VERSION] [--assume-yes] [--skip-update] [--install-all] [--rm-pkgs-pre=p1,p2,...] [--rm-pkgs-post=p1,p2,...]
...
--upgrade-os   : When option is given, Photon OS is upgraded to next release,
                 ...
                 Note: current photon-upgrade version does not support this
--to-ver       : The Photon OS version to upgrade to
                 Note: Photon OS 5.0 is the latest release
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --skip-update )
      ...
      ;;
    --to-ver )
      echoerr "Currently installed Photon OS 5.0 is the latest version." && \
          show_help $ERETRY_EINVAL
      #TO_VERSION="$2"
      shift
      ;;
    --upgrade-os )
      echoerr "Currently installed Photon OS 5.0 is the latest version." && \
          show_help $ERETRY_EINVAL
      #TO_VERSION=6.0   # default Photon OS version to upgrade to, --to-ver
...
[ -n "$TO_VERSION" ] && case "$TO_VERSION" in
  6.0 )
    source ${PHOTON_UPGRADE_UTILS_DIR}/ph5-to-ph6-upgrade.sh
    ;;
  * )
...
# be used during both, upgrading OS or updating installed RPMs to latest version
if [ -n "$INSTALL_ALL" ] && [ -z "$REPOS_OPT" ]; then
  # --install-all is given but --repos was not specified - invalid invocation
  show_help $ERETRY_EINVAL
fi

# custom repos may not provide debuginfo packages hence, removing.
