...
%define falcosecurity_libs_prefix_src %{_arch}-vmware-linux/falcosecurity-libs-repo/falcosecurity-libs-prefix/src
...
Name:           falco
Version:        0.36.2
Release:        18.1.0.1%{?kernelsubrelease}%{?dist}
URL:            https://falco.org
...
Patch2: falcosecurity-libs-use-local-src.patch
...
%autosetup -p1
mkdir -p %{uthash_path}
tar xf %{SOURCE1} --strip-components=1 -C %{_arch}-vmware-linux
mkdir -p %{falcosecurity-libs}
tar xf %{falcosecurity_src} --strip-components=1 -C %{falcosecurity_libs}
cp %{SOURCE2} %{uthash_path}
...
%changelog
* Sat Aug 15 2026 Vamsi Krishna Brahmajosyula <vamsi-krishna.brahmajosyula@broadcom.com> 0.36.2-18.1.0.1
