...
install -d %{buildroot}%{_sharedstatedir}/likewise/db
install -d %{buildroot}%{_sharedstatedir}/likewise/rpc
...
case "$1" in
    1)
    systemctl enable lwsmd.service >/dev/null 2>&1
    if [ $? -ne 0 ]; then
      ln -s /lib/systemd/system/lwsmd.service /etc/systemd/system/multi-user.target.wants/lwsmd.service
    fi
    ...
    if [ $try_starting_lwregd_svc = true ]; then
        [ -z "`pidof lwsmd`" ] && systemctl start lwsmd.service
...
if [ "$1" = 0 ]; then
    ...
    systemctl stop lwsmd.service

    systemctl disable lwsmd.service

    if [ -f /etc/systemd/system/lwsmd.service ]; then
       rm -f /etc/systemd/system/lwsmd.service
    fi
...
%dir %{_sharedstatedir}/likewise
%dir %{_sharedstatedir}/likewise/db
%dir %{_sharedstatedir}/likewise/rpc
