...
%build
# aarch64 91 snapshot is old
%ifarch aarch64
  # Remove stale include-fixed/.. generated by the previously installed GCC
  # against an older glibc.
  files=(pthread.h features.h sys math.h stdlib.h)
  pushd %{_libdir}/gcc/$(gcc -dumpmachine)/$(gcc -dumpfullversion)/include-fixed
  for f in ${files[@]}; do
    if test -e $f && test -e %{_includedir}/$f; then
      echo "Removing: $f"
      rm -r $f
    fi
  done
...
* Tue Jun 17 2025 Vamsi Krishna Brahmajosyula <vamsi-krishna.brahmajosyula@broadcom.com> 12.2.0-7
- Release bump for aarch64 SRP compliance
* Tue Mar 25 2025 Brennan Lamoreaux <brennan.lamoreaux@broadcom.com> 12.2.0-6
- Add plugin callback before struct layout
