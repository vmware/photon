...
ExecStop=/usr/bin/bash -c "[ -f /var/run/wavefront-proxy.pid ] && kill \$(cat /var/run/wavefront-proxy.pid)"
Restart=on-failure

[Install]
WantedBy=multi-user.target
EOF
sed -i 's/\/etc\/init.d\/$APP_BASE-proxy/ systemctl $APP_BASE-proxy/' pkg/opt/wavefront/wavefront-proxy/bin/autoconf-wavefront-proxy.sh
