...
class PackageManager(object):
    ...
    def buildToolChainPackages(self, listBuildOptionPackages, pkgBuildOptionFile, buildThreads):
        ...
        if pkgCount > 0 or not self.dockerClient.images.list("photon_build_container:latest"):
            self.logger.debug("VDBG-PM-buildToolChainPackages: Calling stage1 createBuildContainer. pkgCount: %s", pkgCount)
    ...
    def createBuildContainer(self):
        ...
        self.logger.info("VDBG-PU-createBuildContainer: buildRootPath: " + constants.buildRootPath)
        self.logger.info("VDBG-PU-createBuildContainer: rpmPath: " + constants.rpmPath)
        self.logger.info("VDBG-PU-createBuildContainer: logPath: " + self.logPath)
        try:
            #TODO image name constants.buildContainerImageName
