...
class ChrootUtils(object):
    ...
    def createChroot(self, chrootName):
        ...
        if not returnVal:
            self.logger.error("Unable to create chroot:"+ chrootID +
                              ".Unknown error.")
            return False, None
    ...
    def destroyChroot(self, chrootID):
        ...
        # http://stackoverflow.com/questions/1191374/subprocess-with-timeout
        cmdUtils = CommandUtils()
        returnVal = cmdUtils.runCommandInShell("./clean-up-chroot.py " +
                                               chrootID)
        if not returnVal:
            self.logger.error("Unable to destroy chroot:" + chrootID +
                              ".Unknown error.")
            return False
