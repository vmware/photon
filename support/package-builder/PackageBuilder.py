...
class PackageBuilder(object):
    ...
    def __init__(self, mapPackageToCycles, listAvailableCyclicPackages,
        ...
        self.listNodepsPackages = ["glibc", "gmp", "zlib", "file",
                                   "binutils", "mpfr", "mpc", "gcc", "ncurses",
                                   "util-linux", "groff", "perl", "texinfo", "rpm",
                                   "openssl", "go"]
        ...
        if self.checkIfPackageIsAlreadyBuilt():
            if not constants.rpmCheck:
                self.logger.info("Skipping building the package:" +
                                 self.package)
                return
            elif (constants.rpmCheck and
                  self.package not in constants.testForceRPMS):
        ...
        try:
            ...
            if constants.rpmCheck and self.package in constants.testForceRPMS:
                listDependentPackages.extend(
                    self.findBuildTimeCheckRequiredPackages())
                testPackages = (set(constants.listMakeCheckRPMPkgtoInstall) -
                                set(listInstalledPackages) - set([self.package]))
                listDependentPackages.extend(testPackages)
                listDependentPackages = list(set(listDependentPackages))

            pkgUtils = PackageUtils(self.logName, self.logPath)
            if len(listDependentPackages) != 0:
                self.logger.info("Installing the build time dependent "
                                 "packages......")
                for pkg in listDependentPackages:
                    self.installPackage(pkgUtils, pkg, chrootID,
                                        self.logPath, listInstalledPackages)
                pkgUtils.installRPMSInAOneShot(chrootID, self.logPath)
                self.logger.info("Finished installing the build time dependent "
                                 "packages......")
    ...
    def findBuildTimeRequiredPackages(self):
        listRequiredPackages = (
            SPECS.getData()
            .getBuildRequiresForPackage(self.package))
        return listRequiredPackages

    def findBuildTimeCheckRequiredPackages(self):
        listRequiredPackages = (
            SPECS.getData()
            .getCheckBuildRequiresForPackage(self.package))
        return listRequiredPackages

    def installPackage(self, pkgUtils, package, chrootID, destLogPath,
                       listInstalledPackages):
        if package in listInstalledPackages:
            return
        # mark it as installed -  to avoid cyclic recursion
        listInstalledPackages.append(package)
        self.installDependentRunTimePackages(pkgUtils, package, chrootID,
    ...
    def installDependentRunTimePackages(self, pkgUtils, package, chrootID,
                                        destLogPath, listInstalledPackages):
