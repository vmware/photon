...
class BuildContainer(object):
    ...
    def __init__(self, mapPackageToCycles, listAvailableCyclicPackages,
                 listBuildOptionPackages, pkgBuildOptionFile, logName=None,
                 logPath=None):
        if logName is None:
        ...
        self.listNodepsPackages = ["glibc", "gmp", "zlib", "file", "binutils",
                                   "mpfr", "mpc", "gcc", "ncurses", "util-linux",
                                   "groff", "perl", "texinfo", "rpm", "openssl",
                                   "openssl-devel", "go"]
    ...
    def prepareBuildContainer(self, containerTaskName, packageName,
        ...
        cmdUtils.runCommandInShell("mkdir -p " +
                                   chrootID + constants.topDirPath)
        ...
        mountVols = {
            ...
            constants.logPath + "/" + self.logName: {'bind': constants.topDirPath + "/LOGS",
                                                     'mode': 'rw'},
            chrootID + constants.topDirPath + "/BUILD": {'bind': constants.topDirPath +
                                                                 "/BUILD", 'mode': 'rw'}}
        ...
        try:
            ...
            self.logger.debug("Started Photon build container for task " +
                              containerTaskName + " ID: " +
                              containerID.short_id)
            if not containerID:
                raise Exception("Unable to start Photon build container "
                                "for task " + containerTaskName)
        except Exception as e:
            self.logger.debug("Unable to start Photon build container for "
                              "task " + containerTaskName)
            raise e
    ...
    def findInstalledPackages(self, containerID):
        ...
        listInstalledRPMs = (
            pkgUtils.findInstalledRPMPackagesInContainer(containerID))
    ...
    def buildPackage(self, package):
        ...
        try:
            containerID, chrootID = (
                self.prepareBuildContainer(containerTaskName, package,
                                           isToolChainPackage))
            ...
            if package in constants.perPackageToolChain:
                ...
                tcUtils.installCustomToolChainRPMSinContainer(
                    containerID,
                    constants.perPackageToolChain[package],
                    package)

            listInstalledPackages, listInstalledRPMs = (
                self.findInstalledPackages(containerID))
            ...
            if constants.rpmCheck and package in constants.testForceRPMS:
                ...
                testPackages = (set(constants.listMakeCheckRPMPkgtoInstall)-
                                set(listInstalledPackages)-set([package]))
                listDependentPackages.extend(testPackages)
                listDependentPackages = list(set(listDependentPackages))

            pkgUtils = PackageUtils(self.logName, self.logPath)
            if len(listDependentPackages) != 0:
                self.logger.info("BuildContainer-buildPackage: Installing "
                                 "dependent packages..")
                self.logger.info(listDependentPackages)
                for pkg in listDependentPackages:
                    self.installPackage(pkgUtils, pkg, containerID, destLogPath,
                                        listInstalledPackages, listInstalledRPMs)
                # Special case sqlite due to package renamed from sqlite-autoconf to sqlite
                if ("sqlite" in listInstalledPackages or
                        "sqlite-devel" in listInstalledPackages or
                        "sqlite-libs" in listInstalledPackages):
                    ...
                    if "sqlite-libs" not in listInstalledPackages:
                        self.installPackage(pkgUtils, "sqlite-libs", containerID,
                                            destLogPath, listInstalledPackages, listInstalledRPMs)
                pkgUtils.installRPMSInAOneShotInContainer(containerID,
                                                          destLogPath)
            ...
            pkgUtils.buildRPMSForGivenPackageInContainer(
                ...
                destLogPath)
            self.logger.info("BuildContainer-buildPackage: Successfully built the "
                             "package: " + package)
        except Exception as e:
    ...
    def findBuildTimeRequiredPackages(self, package):
        listRequiredPackages = (
            SPECS.getData().getBuildRequiresForPackage(package))
        return listRequiredPackages

    def findBuildTimeCheckRequiredPackages(self, package):
        listRequiredPackages = (
            SPECS.getData().getCheckBuildRequiresForPackage(package))
        return listRequiredPackages

    def installPackage(self, pkgUtils, package, containerID,
                       destLogPath, listInstalledPackages, listInstalledRPMs):
        latestRPM = os.path.basename(
            pkgUtils.findRPMFileForGivenPackage(package)).replace(".rpm", "")
        if (package in listInstalledPackages and
                latestRPM in listInstalledRPMs):
            return
        self.installDependentRunTimePackages(pkgUtils, package, containerID,
                                             destLogPath, listInstalledPackages,
                                             listInstalledRPMs)
        noDeps = False
        ...
        if package in constants.noDepsPackageList:
            noDeps = True
        pkgUtils.prepRPMforInstallInContainer(package, containerID, noDeps,
                                              destLogPath)
        listInstalledPackages.append(package)
        listInstalledRPMs.append(latestRPM)

    def installDependentRunTimePackages(self, pkgUtils, package, containerID,
                                        destLogPath, listInstalledPackages,
                                        listInstalledRPMs):
        listRunTimeDependentPackages = (
            self.findRunTimeRequiredRPMPackages(package))
        if len(listRunTimeDependentPackages) != 0:
            for pkg in listRunTimeDependentPackages:
                if (pkg in self.mapPackageToCycles and
                        pkg not in self.listAvailableCyclicPackages):
                    continue
                latestPkgRPM = (
                    os.path.basename(pkgUtils.findRPMFileForGivenPackage(pkg)).replace(".rpm", ""))
                if (pkg in listInstalledPackages and
                        latestPkgRPM in listInstalledRPMs):
                    continue
                self.installPackage(pkgUtils, pkg, containerID, destLogPath, listInstalledPackages,
                                    listInstalledRPMs)
