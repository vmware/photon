...
def main():
    ...
    parser.add_argument("-s", "--spec-path",
                        dest="specPath", default="../../SPECS")
    parser.add_argument("-x", "--source-path",
                        dest="sourcePath", default="../../stage/SOURCES")
    parser.add_argument("-r", "--rpm-path",
                        dest="rpmPath", default="../../stage/RPMS")
    parser.add_argument("-i", "--install-package",
                        dest="installPackage", default=False, action="store_true")
    parser.add_argument("-p", "--publish-RPMS-path",
                        dest="publishRPMSPath", default="../../stage/PUBLISHRPMS")
    parser.add_argument("-e", "--publish-XRPMS-path",
                        dest="publishXRPMSPath", default="../../stage/PUBLISHXRPMS")
    parser.add_argument("-l", "--log-path",
                        dest="logPath", default="../../stage/LOGS")
    parser.add_argument("-z", "--top-dir-path",
                        dest="topDirPath", default="/usr/src/photon")
    parser.add_argument("-b", "--build-root-path",
                        dest="buildRootPath", default="/mnt")
    parser.add_argument("-t", "--threads",
                        dest="buildThreads", default=1, type=int, help="Number of working threads")
    parser.add_argument("-m", "--tool-chain-stage",
                        dest="toolChainStage", default="None")
    parser.add_argument("-c", "--pullsources-config",
                        dest="pullsourcesConfig", default="pullsources.conf")
    parser.add_argument("-d", "--dist",
                        dest="dist", default="")
    parser.add_argument("-k", "--input-RPMS-path",
                        dest="inputRPMSPath", default=None)
    parser.add_argument("-n", "--build-number",
                        dest="buildNumber", default="0000000")
    parser.add_argument("-v", "--release-version",
                        dest="releaseVersion", default="NNNnNNN")
    parser.add_argument("-u", "--enable-rpmcheck",
                        dest="rpmCheck", default=False, action="store_true")
    parser.add_argument("-a", "--source-rpm-path",
                        dest="sourceRpmPath", default="../../stage/SRPMS")
    parser.add_argument("-w", "--pkginfo-file",
                        dest="pkgInfoFile", default="../../stage/pkg_info.json")
    parser.add_argument("-g", "--pkg-build-option-file",
                        dest="pkgBuildOptionFile",
                        default="../../common/data/pkg_build_options.json")
    parser.add_argument("-q", "--rpmcheck-stop-on-error",
                        dest="rpmCheckStopOnError", default=False, action="store_true")
    parser.add_argument("-bd", "--publish-build-dependencies",
                        dest="publishBuildDependencies", default=False)
    parser.add_argument("-pw", "--package-weights-path",
                        dest="packageWeightsPath", default=None)
    parser.add_argument("-y", "--generate-pkg-yaml-files",
                        dest="generatePkgYamlFiles", default=False, action="store_true")
    parser.add_argument("-j", "--pkg-yaml-dir-path",
                        dest="pkgYamlDirPath", default="../../stage/")
    parser.add_argument("-f", "--pkg-blacklist-file",
                        dest="pkgBlacklistFile", default=None)
    parser.add_argument("-bt", "--build-type",
                        dest="pkgBuildType", default="chroot")
    parser.add_argument("-F", "--kat-build",
                        dest="katBuild", default=None)
    ...
    try:
        ...
        if package == "packages_list":
            ...
            buildSRPMList(options.sourceRpmPath, options.pkgYamlDirPath,
            ...
            buildAPackage(package, listBuildOptionPackages,
                          options.pkgBuildOptionFile, options.buildThreads,
                          options.pkgBuildType)
        else:
            buildPackagesForAllSpecs(listBuildOptionPackages,
                                     options.pkgBuildOptionFile,
                                     logger, options.buildThreads,
                                     pkgInfoJsonFile,
                                     options.pkgBuildType)
...
def buildPackagesList(csvFilename):
    ...
    for package in listPackages:
        ...
        csvFile.write(name + "," + version + "," + license + "," + url + "," + sources + "," +
                      patches+"\n")
    csvFile.close()
...
def buildSourcesList(yamlDir, blackListPkgs, logger, singleFile=True):
    ...
    for package in listPackages:
        ...
        if not singleFile:
            yamlFile = open(yamlSourceDir + "/" + ossname + "-" +
                            ossversion + ".yaml", "w")
...
def buildSRPMList(srpmPath, yamlDir, blackListPkgs, logger, singleFile=True):
    ...
    for package in listPackages:
        ...
        listFoundSRPMFiles = cmdUtils.findFile(ossname + "-" + ossversion + "-" +
                                               ossrelease + ".src.rpm", srpmPath)
        srpmName = None
...
def buildAPackage(package, listBuildOptionPackages, pkgBuildOptionFile,
    ...
    pkgManager.buildPackages(listPackages, listBuildOptionPackages,
                             pkgBuildOptionFile, buildThreads, pkgBuildType)

def buildPackagesForAllSpecs(listBuildOptionPackages,
                             pkgBuildOptionFile, logger,
                             buildThreads, pkgInfoJsonFile,
                             pkgBuildType):
    listPackages = SPECS.getData().getListPackages()
...
def get_packages_with_build_options(pkg_build_options_file):
    ...
    if os.path.exists(pkg_build_options_file):
        ...
        pkg_build_option_json = json.load(jsonData,
                                          object_pairs_hook=collections.OrderedDict)
        jsonData.close()
        pkgs_sorted = list(pkg_build_option_json.keys())
