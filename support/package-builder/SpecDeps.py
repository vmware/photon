            ...
            for version in SPECS.getData().getVersions(base_package):
                ...
                for package in listRPMPackages:
                    ...
                    filename = os.path.join(
                        buildarch, f"{package}-{version}.{buildarch}.rpm"
                    )
                ...
                if (
                    ...
                    filename = os.path.join(
                        buildarch, f"{base_package}-debuginfo-{version}.{buildarch}.rpm"
                    )
                    output.append(filename)
        ...
        if inputType == "pkg" or inputType == "json":
            ...
            if outputFile is not None:
                return self.displayDependencies(
                    displayOption, inputType, outputFile, mapDependencies, parent
                )
            else:
                return self.displayDependencies(
                    displayOption, inputType, inputValue, mapDependencies, parent
                )
...
def main():
    ...
    parser.add_argument(
        "-d", "--display-option", dest="display_option", default=DEFAULT_DISPLAY_OPTION
    )
    ...
    parser.add_argument(
        "-a", "--input-data-dir", dest="input_data_dir", default="../../common/data/"
    )
    parser.add_argument(
        "-o", "--output-dir", dest="output_dir", default="../../stage/common/data"
    )
