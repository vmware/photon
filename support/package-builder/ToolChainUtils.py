...
class ToolChainUtils(object):
    ...
    def installToolChainRPMSinContainer(self, containerID):
        ...
        for package in constants.listToolChainRPMPkgsToInstall:
            ...
            if rpmFile.find("stage/RPMS"):
                rpmFile = rpmFile.replace(constants.rpmPath, "/toolchainrpms")
            rpmFiles += " " + rpmFile
            packages += " " + package
        ...
        self.logger.info("VDBG-TCU-installToolChainRPMSinContainer: TCInstallLog: " + tcInstallLog)
    ...
    def installCustomToolChainRPMSinContainer(self, containerID, listOfToolChainPkgs, packageName):
        ...
        packages = ""
        for package in listOfToolChainPkgs:
            print "VDBG-TCU-installCustomToolChainRPMSinContainer-DEBUG:" + package
            if "openjre8" in packageName or "openjdk8" in packageName:
                rpmFile=self.findRPMFileInGivenLocation(package, constants.prevPublishXRPMRepo)
            else:
                rpmFile=self.findRPMFileInGivenLocation(package, constants.prevPublishRPMRepo)
    ...
    def installToolChainXRPMSinContainer_10(self, containerID):
        self.logger.info("Installing Tool Chain X package RPMS.......")
        rpmFiles = ""
        packages = ""
        for package in constants.listToolChainXRPMsToInstall:
            pkgUtils = PackageUtils(self.logName, self.logPath)
            self.logger.debug("VDBG-TCU-installToolChainXRPMSinContainer xrpm pkg: " + package)
            rpmFile = self.findRPMFileInGivenLocation(package, constants.prevPublishXRPMRepo)
            if rpmFile is None:
                self.logger.error("Unable to find rpm " + package + " in current and previous versions")
                raise Exception("Input Error")
        ...
        self.logger.debug("Installing rpms: " + packages)
        cmd = "/usr/bin/bash -l -c 'rpm -Uvh --nodeps --force " + rpmFiles + "'"
        self.logger.debug("VDBG-TCU-installCustomToolChainRPMSinContainer: pkg: " + package + ", cmd: '" + cmd + "'")
        #TODO: Parse error log to get actual success/failure
        tcInstallLog = containerID.exec_run(cmd)
        if not tcInstallLog:
            self.logger.error("Installing x-tool-chain in container failed for package: ")
            raise Exception("RPM installation in container failed")
        self.logger.info("VDBG-TCU-installToolChainXRPMSinContainer: TCInstallLog: " + tcInstallLog)
        self.logger.info("Successfully installed all tool-chain customRPMS/XRPMS in container: " + containerID.short_id)

    def installToolChainRPMSinContainer_10(self, containerID):
        self.logger.info("Installing tool-chain RPMS in container: " + containerID.short_id)
        rpmFiles = ""
        packages = ""
        pkgUtils = PackageUtils(self.logName, self.logPath)
        for package in constants.listToolChainRPMsToInstall:
            ...
            if rpmFile is None:
                ...
                if rpmFile is None:
                    ...
                    self.logger.error("Unable to find rpm " + package + " in current and previous versions")
                    raise Exception("Input Error")
            #TODO: get <buildroot> from constants
            if rpmFile.find("stage/PUBLISHRPMS"):
                rpmFile = rpmFile.replace("/root/gp2/stage/PUBLISHRPMS", "/publishrpms")
            if rpmFile.find("stage/PUBLISHXRPMS"):
                rpmFile = rpmFile.replace("/root/gp2/stage/PUBLISHXRPMS", "/publishxrpms")
            if rpmFile.find("stage/RPMS"):
                rpmFile = rpmFile.replace("/root/gp2/stage/RPMS", "/toolchainrpms")
            rpmFiles += " " + rpmFile
            packages += " " + package

        self.logger.debug("Installing rpms: " + packages)
        self.logger.info("VDBG-TCU-installToolChainRPMSinContainer: Installing rpmfiles: " + rpmFiles)

        cmd = "rpm -ivh --force --nodeps " + rpmFiles
        tcInstallLog = containerID.exec_run(cmd)
        if not tcInstallLog:
            self.logger.error("Installing tool chain in container failed")
            raise Exception("RPM installation in container failed")
        ...
        #if packageName in constants.perPackageToolChain:
        #    print constants.perPackageToolChain[packageName]
        #    self.installCustomToolChainRPMS(containerID,
        #                                    constants.perPackageToolChain[packageName],
        #                                    packageName);
