...
class ToolChainUtils(object):
    ...
    def prepareBuildRoot(self, chrootID):
        ...
        if not returnVal:
            self.logger.error('Prepare build root script failed.Unable '
                              'to prepare chroot.')
    ...
    def findRPMFileInGivenLocation(self, package, rpmdirPath):
        ...
        if len(listFilterRPMFiles) == 0:
            return None
        if len(listFilterRPMFiles) > 1:
            self.logger.error('Found multiple rpm files for given package '
                              'in rpm directory.Unable to determine the rpm '
    ...
    def installToolChainRPMS(self, chrootID, packageName, logPath=None):
            ...
            if rpmFile is None:
                ...
                if rpmFile is None:
                    if package in constants.listOfRPMsProvidedAfterBuild:
                        self.logger.info('No old version of ' +
                                         package + ' exists, skip until the new version '
                                         'is built')
                        continue
        ...
        self.logger.info('Successfully installed default Tool Chain RPMS in '
                         'Chroot:' + chrootID)
    ...
    def installCustomToolChainRPMS(self, chrootID, listOfToolChainPkgs,
        ...
        for package in listOfToolChainPkgs:
            ...
            if "openjre8" in packageName or "openjdk8" in packageName:
                ...
                rpmFile = self.findRPMFileInGivenLocation(package,
                                                          constants.prevPublishRPMRepo)
            if rpmFile is None:
                self.logger.error("Unable to find rpm "+
                                  package +" in current and previous versions")
                raise Exception("Input Error")
            rpmFiles += " " + rpmFile
            packages += " " + package
...
#                    package = "sqlite-autoconf"
        ...
        cmd = ("/usr/bin/bash -l -c '/usr/bin/rpm -Uvh --force --nodeps " +
               rpmFiles + "'")
        self.logger.info("VDBG-TCU-installToolChainRPMSinContainer: "
                         "Installing rpms cmd: " + cmd)
        ...
        self.logger.info("Successfully installed default tool-chain RPMS in "
                         "container: " + containerID.short_id)

    def installCustomToolChainRPMSinContainer(self, containerID,
                                              listOfToolChainPkgs,
                                              packageName):
        self.logger.info("Installing package specific tool chain RPMs for " +
                         packageName)
        ...
        self.logger.debug("VDBG-TCU-installCustomToolChainRPMSinContainer: "
                          "Installing rpms cmd: " + cmd)
        tcInstallLog = containerID.exec_run(cmd)
        # TODO: Find a way to collect exit status of the command that was run.
        if not tcInstallLog:
            ...
            raise Exception("RPM installation in container failed")
        self.logger.info(tcInstallLog)
        self.logger.info("Successfully installed all tool-chain XRPMS "
