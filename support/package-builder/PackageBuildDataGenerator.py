...
class PackageBuildDataGenerator(object):
    ...
    def getPackageBuildData(self, listPackages):
        ...
        return (self.__mapCyclesToPackageList, self.__mapPackageToCycle,
                self.__sortedPackageList)
    ...
    def __getSortedBuildOrderListForGivenPackages(self, listPackages):
        ...
        sortedList = []
        completeListPackagesToBuild = (
            self.findCompleteListOfPackagesRequiredToBuildGivenPackages(
                listPackages))
    ...
    def __readDependencyGraphAndCyclesForGivenPackages(self, listPackages):
        ...
        for pkg in self.__buildDependencyGraph.keys():
            sortedPackageList, circularDependentPackages = (
                self.topologicalSortPackages(self.__buildDependencyGraph, pkg))
        ...
        sortedPackageList, circularDependentPackages = (
            self.topologicalSortPackages(self.__runTimeDependencyGraph))
        if len(circularDependentPackages) > 0:
    ...
    def topologicalSortPackages(self, dependencyGraph, package=None):
        ...
        #Find packages with no dependencies and generate dependentof_package
        #edge list
        ...
        #return (non-circular dependent package in sorted order and circular
        # dependent package list in a dependencyGraph)
        return sortedPackageList, circularDependencyGraph

    def __constructDependencyMap(self, cyclicDependencyGraph):
        self.logger.info("Constructing dependency map from circular "
                         "dependency graph.....")
    ...
    def __findCircularDependencies(self, cyclicDependencyGraph):
        ...
        constructDependencyMap = (
            self.__constructDependencyMap(cyclicDependencyGraph))

        #step2: find cycles in dependency map
        self.logger.info("Finding and adding cycles using constructed "
                         "dependency map......")
        cycleCount = 0
        listNodes = cyclicDependencyGraph.keys()
        for node in listNodes:
        ...
        if cycleCount > 0:
            ...
            self.logger.info("Successfully added all detected circular "
