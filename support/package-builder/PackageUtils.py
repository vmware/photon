...
#        if os.geteuid()==0:
    ...
    def copyAdditionalBuildFilesToContainer(self, listAdditionalFiles, containerID):
        ...
        for additionalFile in listAdditionalFiles:
            ...
            self.logger.debug("VDBG-PU-copyAdditionalBuildFilesToContainer: copying additional file - source: " + source + " , destDir: " + destDir)
            #TODO: exit status of exec_run
            eSts = containerID.exec_run("mkdir -p " + destDir)
            self.logger.debug("VDBG-PU-copyAdditionalBuildFilesToContainer eSts: " + eSts)
            if os.path.exists(source):
                if os.path.isfile(source):
                    self.logger.info("Copying addl source file: " + source)
                    copyCmd = "docker cp " + source + " " + destPath
                    #TODO: cmd error code
                    self.logger.debug("VDBG-PU-copyAdditionalBuildFilesToContainer cpCmd: " + copyCmd)
                    cmdUtils.runCommandInShell(copyCmd)
                else:
    ...
    def buildRPMSForGivenPackageInContainer(self, package, containerID, listBuildOptionPackages,
        ...
        cpSpecCmd = "docker cp " + specFile + " " + containerID.short_id \
                        + ":" + specPath + specName
        self.logger.debug("VDBG-PU-buildRPMSForGivenPackageInContainer cpSpecCmd: " + cpSpecCmd)
...
#        if os.geteuid()==0:
        ...
        for macroName in listRPMMacros.keys():
            macros.append(macroName + " " + listRPMMacros[macroName])
        self.logger.debug("VDBG-PU-buildRPMSForGivenPackageInContainer macros: ")
        self.logger.debug(macros)
