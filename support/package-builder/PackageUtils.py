...
#        self.logger.debug("No deps:"+str(noDeps))
    ...
    def getAdditionalBuildFiles(self, pkgBuildOptionFile):
        listAdditionalFiles = []
        macros = []
...
#        if os.geteuid()==0:
        ...
        if package in listBuildOptionPackages:
            listAdditionalFiles, macros = getAdditionalBuildFiles(pkgBuildOptionFile)
    ...
    def copyAdditionalBuildFilesToContainer(self, listAdditionalFiles, containerID):
        ...
        for additionalFile in listAdditionalFiles:
            ...
            if os.path.exists(source):
                self.logger.info("Copying source file: " + source)
                copyCmd = "docker cp " + source + " " + containerID.short_id + ":" + destPath
                cmdUtils.runCommandInShell(copyCmd)
                self.logger.info("VDBG-PU-copyAdditionalBuildFilesToContainer cpCmd: " + copyCmd)
...
#        if os.geteuid()==0:
    ...
    def buildRPMinContainer(self, specFile, rpmLogFile, destLogFile, containerID, package, macros):
        ...
        self.logger.debug("VDBGG: Building rpm cmd: " + rpmBuildCmd)
