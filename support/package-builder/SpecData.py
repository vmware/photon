...
class SerializableSpecObjectsUtils(object):
    ...
    def readSpecsAndConvertToSerializableObjects(self, specFilesPath):
        ...
        for specFile in listSpecFiles:
            ...
            specObj.checkBuildRequirePackages = (
                spec.getCheckBuildRequiresAllPackages())
            ...
            for specPkg in specObj.listPackages:
                if specPkg in self.mapPackageToSpec:
                    existingObj = (
                        self.mapSerializableSpecObjects[self.mapPackageToSpec[specPkg]])
                    if self.compareVersions(existingObj, specObj) == 1:
                        skipUpdating = True
                        break
                specObj.installRequiresPackages[specPkg] = (
                    spec.getRequires(specPkg))
...
class SPECS(object):
    ...
    def initialize(self):
        ...
        kernelsubrelease = ('%02d%02d%03d%03d' % (int(a), int(b), int(c),
...
class SerializedSpecObjects(object):
    ...
    def findTotalWhoNeedsToBuild(self, depQue, whoBuildDeps, whoBuildDepSet,
                                 displayOption):
        while not depQue.empty():
            specPkg = depQue.get()
            specName = self.getSpecName(specPkg)
    ...
    def readSpecsAndConvertToSerializableObjects(self, specFilesPath,
            ...
            for pkg in data:
                if pkg not in allDeps:
                    ...
                    if spName != None:
                        ...
                        parent[pkg] = ""
                        depQue.put(pkg)
                        self.findTotalRequires(allDeps, depQue, parent,
                                               displayOption)
                    else:
                        independentRPMS.append(pkg)

        #Generating the list of packages that requires the given input
        #package at install time
        elif inputType == "who-needs":
            print(whoNeedsList)
            return

        #Generating the list of packages that the modified package will
        #affect at build time
        elif inputType == "who-needs-build":
            if packageFound == True:
                self.findTotalWhoNeedsToBuild(depQue, whoBuildDeps,
                                              whoBuildDepSet, displayOption)
        ...
        sortedList = []
        for elem in sorted(allDeps.items(), key=operator.itemgetter(1),
                           reverse=True):
    ...
    def getSpecName(self, package):
        if package in self.mapPackageToSpec:
            ...
            if specName in self.mapSerializableSpecObjects:
                ...
                print("SpecDeps: Could not able to find " +
                      package + " package from specs")
                raise Exception("Invalid package:" + package)
        else:
            return None
