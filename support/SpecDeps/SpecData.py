...
class SerializedSpecObjects(object):
    ...
    def findTotalWhoBuildRequires(self, depQue, whoBuildDeps, whoBuildDepSet, displayOption):
        while not depQue.empty():
            specPkg = depQue.get()
            specName = self.getSpecName(specPkg)
    ...
    def readSpecsAndConvertToSerializableObjects(self, specFilesPath, inputType, inputValue, displayOption):
        ...
        #Generating the list of packages that the given input package requires at build time
        elif (inputType == "build-deps"):
            if (packageFound == True):
                self.findTotalWhoBuildRequires(depQue, whoBuildDeps, whoBuildDepSet, displayOption)
