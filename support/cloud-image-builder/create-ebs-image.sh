...
export AWS_ACCESS_KEY_ID=AKIAI5XIK6QXV7XVKCVA
export AWS_SECRET_ACCESS_KEY=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
export AWS_DEFAULT_REGION=us-west-2
#echo -e "AKIAIVB52XJP4ADTVPVQ\nbSwgbkL/QD6Zhhf6+2AGkA+jgJJUkQpyMFl8TcAM\nus-west-2\n\n" | aws configure
...
aws ec2 attach-volume --volume-id $VOLUMEID_COPY --instance-id $INSTANCE_ID --device $COPY_DEVICE --region $AWS_DEFAULT_REGION
...
aws ec2 attach-volume --volume-id $VOLUMEID --instance-id $INSTANCE_ID --device $DEVICE --region $AWS_DEFAULT_REGION

#Copy the image file
ssh -i $KEY_FILE root@$IP 'rm -rf /mnt/copy; rm -rf /mnt/ebs; mkdir /mnt/copy; mkdir /mnt/ebs'
ssh -i $KEY_FILE root@$IP "mkfs.ext4 $COPY_DEVICE"
ssh -i $KEY_FILE root@$IP "mount $COPY_DEVICE /mnt/copy"
scp -i $KEY_FILE $TAR_FILE.tar.gz root@$IP:/mnt/copy
ssh -i $KEY_FILE root@$IP "cd /mnt/copy; tar -xf $TAR_FILE.tar.gz; dd if=$TAR_FILE.raw of=$DEVICE bs=1M"
ssh -i $KEY_FILE root@$IP 'umount /mnt/copy'
ssh -i $KEY_FILE root@$IP 'rm -rf /mnt/copy /mnt/ebs'

#detach the volume
aws ec2 detach-volume --volume-id $VOLUMEID --region $AWS_DEFAULT_REGION
aws ec2 detach-volume --volume-id $VOLUMEID_COPY --region $AWS_DEFAULT_REGION
