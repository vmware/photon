...
with open(
    os.path.dirname(os.path.realpath(__file__)) + f"/{cfg_fn}", "r"
) as f:
...
def check_spec_cfg_yml(spec_fn, specDir, err_dict):
...
def check_entire_cfg_ymls(specDir):
    checker = SourceArchiveChecker()
    checker.scanDirectory(specDir)
    return checker.checkConflicts()
...
def check_specs(files_list):
    ...
    for spec_fn in files_list:
        ...
        if any(
            [
                ...
                check_spec_cfg_yml(altered_spec, currSpecDir, err_dict),
            ]
        ):
            err = True
    ...
    allSpecDir = os.path.dirname(os.path.dirname(files_list[0]))
    retVal, logs = check_entire_cfg_ymls(allSpecDir)
    if retVal:
        ret = True
