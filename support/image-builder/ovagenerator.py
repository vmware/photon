...
def create_ova_image(raw_image_name, tools_path, config):
    ...
    cmdUtils.runBashCmd("ovftool {} {}".format(vmx_path, ovf_path))
    ...
    for num_img in range(len(raw_image_name)):
        cmd = "openssl sha1 {}-disk{}.vmdk {}.ovf".format(image_name, num_img + 1, image_name)
        out, _, _ = cmdUtils.runBashCmd(cmd, capture=True)
    ...
    if "additionalhwversion" in config:
        for addlversion in config["additionalhwversion"]:
            ...
            for num_img in range(len(raw_image_name)):
                cmd = "openssl sha1 {}-disk{}.vmdk".format(image_name, num_img + 1)
                cmd += " {}-hw{}.ovf".format(image_name, addlversion)
                out, _, _ = cmdUtils.runBashCmd(cmd, capture=True)
                with open(mf_path, "w") as source:
                    source.write(out)
