...
sudo docker run -it --privileged -v ${SRCROOT}:/photon -v /var/log:/var/log -v $(pwd)/stage/RPMS:/RPMS -v $(pwd)/stage/ostree-repo:/srv/rpm-ostree -w="/photon/installer"  vmware/photon:rpm-ostree ./mk-ostree-server.sh /
