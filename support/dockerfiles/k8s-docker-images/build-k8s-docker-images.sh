...
#
# Major:Minor version
for K8S_MM in 1.9 1.10
do
    K8S_VER=`cat ${SPEC_DIR}/kubernetes/kubernetes-${K8S_MM}.spec | grep "^Version:" | cut -d: -f2 | tr -d ' '`
    K8S_VER_REL=${K8S_VER}-`cat ${SPEC_DIR}/kubernetes/kubernetes-${K8S_MM}.spec | grep "^Release:" | cut -d: -f2 | tr -d ' ' | cut -d% -f1`
