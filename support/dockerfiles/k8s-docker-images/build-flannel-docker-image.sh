...
#
setup_repo
for file in ${SPEC_DIR}/flannel/flannel-*.spec; do
    FLANNEL_VER=`cat ${file} | grep Version | cut -d: -f2 | tr -d ' '`
    FLANNEL_VER_REL=${FLANNEL_VER}-`cat ${file} | grep Release | cut -d: -f2 | tr -d ' ' | cut -d% -f1`
