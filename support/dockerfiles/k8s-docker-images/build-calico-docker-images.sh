...
fn="${SPEC_DIR}/kube-controllers/kube-controllers.spec"
KUBE_CONTROLLERS_VER=$(get_spec_ver "${fn}")
KUBE_CONTROLLERS_VER_REL=${KUBE_CONTROLLERS_VER}-$(get_spec_rel "${fn}")
KUBE_CONTROLLERS_RPM=kube-controllers-${KUBE_CONTROLLERS_VER_REL}${DIST_TAG}.${ARCH}.rpm
KUBE_CONTROLLERS_RPM_FILE=${STAGE_DIR}/RPMS/$ARCH/${KUBE_CONTROLLERS_RPM}
...
if [ ! -f ${KUBE_CONTROLLERS_RPM_FILE} ]; then
...
CALICO_CNI_TAR=calico-cni-v${CALICO_VER_REL}.tar
CALICO_K8S_POLICY_TAR=calico-k8s-policy-v${CALICO_VER_REL}.tar
