...
#
for file in ${SPEC_DIR}/calico/calico-*.spec; do
    CALICO_VER=`cat ${file} | grep Version | cut -d: -f2 | tr -d ' '`
    CALICO_VER_REL=${CALICO_VER}-`cat ${file} | grep Release | cut -d: -f2 | tr -d ' ' | cut -d% -f1`
