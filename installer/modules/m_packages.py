...
def execute(name, ks_config, config, root):
    ...
    if ks_config:
        ...
        if ks_config['type'] == 'micro':
            packages = package_list_micro["micro_packages"]
        elif ks_config['type'] == 'minimal':
            packages = package_list_minimal["minimal_packages"]
        elif ks_config['type'] == 'full':
            packages = package_list_minimal["minimal_packages"] + package_list_full["optional_packages"]
        else:
            #TODO: error
            packages = []
