...
class SelectDisk(object):
    def __init__(self,  maxy, maxx, install_config):
        ...
        self.window = Window(self.win_height, self.win_width, self.maxy, self.maxx, 'Select a disk', True, items = self.disk_buttom_items, menu_helper = self.save_index, position = 1)
        ...
        if 'partitionsnumber' in self.install_config:
                if (int(self.install_config['partitionsnumber']) == 0):
                    ...
                    for i in range (int (self.install_config['partitionsnumber'])):
                        sizedata = int(self.install_config[str(i)+'partition_info'+str(1)])
                        mtdata = self.install_config[str(i)+'partition_info'+str(3)]
                        typedata = self.install_config[str(i)+'partition_info'+str(2)]
                        devicedata = self.install_config[str(i)+'partition_info'+str(0)]

                        partitions = partitions + [
                                    {"mountpoint": mtdata, "size": sizedata, "filesystem": typedata},
                                    ]
                    partitions_data = modules.commons.partition_disk(devicedata, partitions)
        else: 
            partitions_data = modules.commons.partition_disk(self.devices[device_index].path, modules.commons.default_partitions)
