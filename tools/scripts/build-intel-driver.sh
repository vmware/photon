...
PHOTON_DIR="$(jq -r '."release-branch-path"' ${SCRIPT_DIR}/../../build-config.json)"
TMP_TXT="$(mktemp)"
...
parse_args() {
    ...
    SPEC_DIR="${PHOTON_DIR}/SPECS/kernels-drivers-intel"
    BUILD_DIR="${SPEC_DIR}/tmp-build-dir"
    OUT_DIR="${PHOTON_DIR}/stage/SANDBOX_DRIVER_RPMS"
...
main() {
    ...
    pushd "$PHOTON_DIR" &> /dev/null || error "Failed to push directory!"
    ...
    KERNEL_VERSION="$KVER" source "${SCRIPT_DIR}"/create-kernel-deps-specs-from-template.sh "$PHOTON_DIR" "$SCRIPT_DIR/../../"
