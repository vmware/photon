...
class DockerUtil:
    ...
    def build_scan_docker_cmd(
        ...
        tool_cmd.extend(["scan", f"--path={docker_scan_mnt}/{relative_path}"])

        if redis_host:
            tool_cmd.append(f"--redis_host={redis_host}")
            tool_cmd.append(f"--redis_port={redis_port}")
            tool_cmd.append(f"--redis_ttl={redis_ttl}")
        ...
        if yaml:
            ...
            tool_cmd.append(f"--yaml={docker_yaml_mnt}/{os.path.basename(yaml)}")

        if cpus:
            tool_cmd.append(f"--cpus={cpus}")

        # docker image is pre-trimmed, no trimming needed here
        tool_cmd.append("--no_trim")
    ...
    def build_validate_docker_cmd(self, file=None, stdin=None):
        ...
        if file and file.endswith(".spec"):
            ...
            local_scan_path = os.path.dirname(local_scan_path)
            docker_mnt_cmd = "--mount "
            docker_mnt_cmd += ",".join(
                [
                    "type=bind",
                    f"src={local_scan_path}",
                    f"target={docker_spec_mnt}",
                    "readonly",
                ]
            )
            ...
            docker_mnt_cmd = "--mount "
            docker_mnt_cmd += ",".join(
                ["type=bind", f"src={file}", f"target={file_mnt_path}", "readonly"]
            )
    ...
    def run_docker_cmd(self, cmd=None, mount_list=[]):
        ...
        supported = False
        for sup_cmd in self._supported_cmds:
            if cmd[0] == sup_cmd:
                supported = True
                break
        ...
        full_cmd = docker_prefix
        for mnt_cmd in mount_list:
            full_cmd += f" {mnt_cmd}"

        full_cmd += f" {self._docker_img_name} {tool_prefix}"
        ...
        result = run_cmd(full_cmd, ignore_rc=False)
